...
export default function makePlayer(k, posVec2, speed) {
  ...
  player.onUpdate(() => {
    ...
    if (isMouseDown) {
      /*subtract player pos from worldMousePos to calculate direction from the mouse to the player
        then we normalize the vector2 with a unit so the value is between -1 and 1 so we can scale
        according to speed*/
